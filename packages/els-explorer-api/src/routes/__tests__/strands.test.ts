...
const sampleStrandRow = {
  id: 20,
  domain_id: 10,
  code: "S1",
  name: "Strand 1",
  description: "A strand",
...
describe("PUT /api/strands/:id", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("updates a strand and returns the mapped result", async () => {
    mockedUpdateRow.mockResolvedValue(sampleStrandRow as never);

    const app = createApp();
    const res = await app.request("/api/strands/20", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Updated Strand", code: "S1-U" }),
    });
    ...
    expect(mockedUpdateRow).toHaveBeenCalledWith(
      "strands",
      20,
      { name: "Updated Strand", code: "S1-U" },
      { edited_at: "NOW()", edited_by: "editor@test.com" },
    );
  });

  it("returns 404 when strand not found", async () => {
    mockedUpdateRow.mockResolvedValue(null);

    const app = createApp();
    const res = await app.request("/api/strands/999", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 400 for invalid id", async () => {
    const app = createApp();
    const res = await app.request("/api/strands/abc", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 400 for invalid body (code too long)", async () => {
    ...
    const res = await app.request("/api/strands/20", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code: "A".repeat(31) }),
    });
  ...
  it("passes only provided fields to updateRow", async () => {
    ...
    const app = createApp();
    await app.request("/api/strands/20", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
describe("DELETE /api/strands/:id", () => {
  ...
  it("cascade deletes strand and all children", async () => {
    mockedQueryOne.mockResolvedValue({ id: 20 } as never);
    mockedQuery.mockResolvedValue({ rows: [], rowCount: 0 } as never);
    mockedDeleteRow.mockResolvedValue(true);

    const app = createApp();
    const res = await app.request("/api/strands/20", { method: "DELETE" });

    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.success).toBe(true);

    // Verify cascade order: indicators, sub_strands, then strand
    ...
    expect(mockedDeleteRow).toHaveBeenCalledWith("strands", 20);
  });
  ...
  it("returns 404 when strand not found", async () => {
    ...
    const app = createApp();
    const res = await app.request("/api/strands/999", { method: "DELETE" });
  ...
  it("returns 400 for invalid id", async () => {
    const app = createApp();
    const res = await app.request("/api/strands/abc", { method: "DELETE" });
...
describe("PATCH /api/strands/:id/verify", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("sets verification to true with timestamp and user", async () => {
    ...
    const app = createApp();
    const res = await app.request("/api/strands/20/verify", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 404 when strand not found", async () => {
    ...
    const app = createApp();
    const res = await app.request("/api/strands/999/verify", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ humanVerified: true }),
    });
  ...
  it("returns 400 for invalid id", async () => {
    const app = createApp();
    const res = await app.request("/api/strands/abc/verify", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ humanVerified: true }),
    });
